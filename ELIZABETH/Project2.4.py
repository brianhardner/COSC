...
def find_best_gain(data):
    ...
    for i in range (1,(len(data[0]))):      #for each field in the data
        d = histogram(data,0,i)             #count up died/survived
        #for value in d:                     #look at each attribute to see if the counts look like leafs
        for value in list(d.keys()):
            if (d[value][0]/(d[value][0] + d[value][1]) >=.8 
            or d[value][1]/(d[value][0] + d[value][1]) >= .8
            or len(d)==1):
                print('this is a leaf node',data[0][i],':',value)
                del d[value]               #if you find a leaf, take it out of the list so we dont try to split it later
                        
        if len(d) == 1:
            print('this is a leaf node',data[0][i],':',d)
            print('returning')
            return
